...
@Component({
  selector: 'formc-ih-activation',
  templateUrl: './ih-activation.component.html',
  styleUrls: ['./ih-activation.component.scss']
})
export class IhActivationComponent implements OnInit {
  ...
  navToUpdPswd()
  {
    this.signinForm.value.userId=this.ls.getItem('verifiedUser');
    this.signinForm.value.mobile_no=this.mobileNumToSend;
    const regnDetails = this.signinForm.value;

    this.jwtAuth.userActivationDetails(regnDetails).subscribe(
      (data: any) => {
        this.loading = false;
        Swal.fire({
          title: 'User Activation Successful. Click ok to login',
          icon: 'success',
